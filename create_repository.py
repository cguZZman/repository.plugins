...
Repository:
    ...
    create_repository.py  --datadir=~/html/software/kodi  --compressed  https://github.com/chadparry/kodi-repository.chad.parry.org.git:repository.chad.parry.org \
...
def fetch_addon_from_folder(raw_addon_location, target_folder):
    ...
    addon_metadata = parse_metadata(metadata_path)
    addon_target_folder = os.path.join(target_folder, addon_metadata.id)
    samepath = os.path.realpath(addon_location) == os.path.realpath(addon_target_folder)
    # Create the compressed add-on archive.
    if not os.path.isdir(addon_target_folder):
        os.mkdir(addon_target_folder)
    zip_filename = get_archive_basename(addon_metadata)
    archive_path = os.path.join(addon_target_folder, zip_filename)
    ...
    with zipfile.ZipFile(
        ...
        for (root, dirs, files) in os.walk(addon_location):
            ...
            for relative_path in files:
                if relative_path[0] == '.' or get_extension(relative_path) == 'pyc' or (samepath and relative_path == zip_filename): continue
                archive.write(os.path.join(root, relative_path), os.path.join(relative_root, relative_path))
    generate_checksum(archive_path)
    if not samepath:
        copy_metadata_files(
